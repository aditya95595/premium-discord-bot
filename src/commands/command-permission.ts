...
import { addCommandRole, formatCommandRoles, getCommandRoleIds, normalizeCommandName, removeCommandRole, resetCommandRoles, validateCommandName, validateRoleId } from '../utils/command-permissions';

const data = new SlashCommandBuilder()
  .setName('command-permission')
  .setDescription('Owner-only: control which roles can use commands')
...
function roleFromPrefix(raw: string | undefined): string | null {
  if (!raw) return null;
  const mention = raw.match(/^<@&(\d{15,25})>$/);
  return mention?.[1] ?? validateRoleId(raw);
}
...
function run(source: any, sub: string, rawCommand: string, roleId?: string) {
  ...
  if (sub === 'view') {
    const roles = getCommandRoleIds(source.guild.id, command);
    return roles.length
      ? `**${command}** → ${roles.map(id => `<@&${id}>`).join(', ')}\nOnly these roles can use it, subject to the command's normal Discord permissions.`
      : `**${command}** → No custom role restriction. Anyone who passes the command's normal Discord permissions can use it.`;
  }
  if (sub === 'reset') {
    const removed = resetCommandRoles(source.guild.id, command);
    return removed ? `Reset **${command}**. Removed ${removed} role restriction${removed === 1 ? '' : 's'}.` : `**${command}** already has no custom role restrictions.`;
  }
...
async function executeSlash(i: any) {
  try {
    ...
    return i.reply({ content: run(i, sub, i.options.getString('command', true), i.options.getRole('role')?.id), ephemeral: true });
  } catch (e) {
    return i.reply({ content: e instanceof Error ? e.message : 'Invalid request.', ephemeral: true });
  }
}
...
async function executePrefix(m: any, args: string[]) {
  ...
  const role = roleFromPrefix(args[2]);
  if (!['add', 'remove', 'reset', 'view'].includes(sub) || !command || ((sub === 'add' || sub === 'remove') && !role)) {
    return m.reply('Usage: !command-permission <add|remove|reset|view> <command> [@role]');
  }
  try {
    return m.reply(run(m, sub, command, role ?? undefined));
  } catch (e) {
    return m.reply(e instanceof Error ? e.message : 'Invalid request.');
  }
}
