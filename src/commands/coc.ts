...
async function executePrefix(m: any, args: string[]) {
  if (!cocConfigured()) return m.reply('⚠️ CoC API is not configured.');
  const sub = (args[0] || 'clan').toLowerCase();
  const valid = new Set(['clan', 'player', 'war', 'warlog', 'cwl', 'members', 'recruit']);
  if (!valid.has(sub)) return m.reply('Usage: !coc <clan|player|war|warlog|cwl|members|recruit> [tag]');
  const tag = args[1] || process.env.COC_DEFAULT_CLAN_TAG || '';
  if (!tag && !['player', 'recruit'].includes(sub)) return m.reply('Provide a clan tag or configure COC_DEFAULT_CLAN_TAG.');
  try {
    if (sub === 'clan') { const c = await getClan(tag); return m.reply({ embeds: [new EmbedBuilder().setTitle(`🏰 ${c.name}`).setDescription(`${c.tag} • Level ${c.clanLevel}`).addFields({ name: 'Members', value: `${c.members}/50`, inline: true }, { name: 'War League', value: c.warLeague?.name || '—', inline: true }, { name: 'War Wins', value: String(c.warWins ?? 0), inline: true })] }); }
    if (sub === 'player' || sub === 'recruit') { const p = await getPlayer(args[1]); if (sub === 'player') return m.reply({ embeds: [new EmbedBuilder().setTitle(`👤 ${p.name}`).setDescription(`${p.tag} • TH${p.townHallLevel}`).addFields({ name: 'Trophies', value: String(p.trophies ?? 0), inline: true }, { name: 'War Stars', value: String(p.warStars ?? 0), inline: true }, { name: 'Heroes', value: heroSummary(p), inline: false })] }); const th = Number(p.townHallLevel || 0), heroes = (p.heroes || []).filter((h: any) => h.village === 'home'), heroLevels = heroes.reduce((n: number, h: any) => n + Number(h.level || 0), 0), score = Math.min(100, Math.round(th * 4 + heroLevels * .45 + Math.min(20, Number(p.warStars || 0) / 50))); return m.reply({ embeds: [new EmbedBuilder().setTitle(`🔎 Recruitment Check • ${p.name}`).setDescription(`Score **${score}/100** • ${score >= 75 ? '🟢 Excellent' : score >= 55 ? '🟡 Review' : '🔴 Weak'}`).addFields({ name: 'Town Hall', value: `TH${th}`, inline: true }, { name: 'Hero levels', value: String(heroLevels), inline: true }, { name: 'War stars', value: String(p.warStars ?? 0), inline: true })] }); }
    if (sub === 'war') return m.reply({ embeds: [fmtWar(await getCurrentWar(tag))] });
    if (sub === 'warlog') { const w = await getWarLog(tag); const rows = (w.items || []).slice(0, 5).map((x: any) => `${x.result === 'win' ? '🟢' : x.result === 'lose' ? '🔴' : '⚪'} ${x.clan?.name || 'Clan'} vs ${x.opponent?.name || 'Opponent'} • ${x.clan?.stars ?? 0}-${x.opponent?.stars ?? 0}`).join('\n') || 'No war history available.'; return m.reply({ embeds: [new EmbedBuilder().setTitle('📜 Recent War Log').setDescription(rows)] }); }
    if (sub === 'members') { const c = await getClan(tag); const rows = (c.memberList || []).slice(0, 25).map((x: any) => `**TH${x.townHallLevel}** ${x.name} • ${x.trophies ?? 0} 🏆`).join('\n') || 'No members returned.'; return m.reply({ embeds: [new EmbedBuilder().setTitle(`👥 ${c.name} Roster`).setDescription(rows)] }); }
    const group = await getCwlLeague(tag); const clans = (group.clans || []).map((c: any) => `${c.tag} • ${c.name} • Level ${c.clanLevel ?? '?'}`).join('\n') || 'No CWL group available.'; return m.reply({ embeds: [new EmbedBuilder().setTitle(`🏆 CWL Group • ${group.season || 'Current'}`).setDescription(clans)] });
