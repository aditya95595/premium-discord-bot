...
async function renderAction(sub: string, tag: string, playerTag: string, reply: (payload: any) => any) {
  if (!tag && sub !== 'player' && sub !== 'recruit') return reply({ content: 'Provide a clan tag or configure `COC_DEFAULT_CLAN_TAG`.' });
  if (sub === 'clan') { const c = await getClan(tag); const e = new EmbedBuilder().setTitle(`🏰 ${c.name}`).setDescription(`${c.tag} • Level ${c.clanLevel}`).setThumbnail(c.badgeUrls?.medium || '').addFields({ name: 'Members', value: `${c.members}/50`, inline: true }, { name: 'War League', value: c.warLeague?.name || '—', inline: true }, { name: 'War Wins', value: String(c.warWins ?? 0), inline: true }, { name: 'Capital League', value: c.capitalLeague?.name || '—', inline: true }, { name: 'Capital Points', value: String(c.clanCapitalPoints ?? 0), inline: true }); return reply({ embeds: [e] }); }
  if (sub === 'player' || sub === 'recruit') { const p = await getPlayer(playerTag); if (sub === 'player') { const e = new EmbedBuilder().setTitle(`👤 ${p.name}`).setDescription(`${p.tag} • TH${p.townHallLevel}`).addFields({ name: 'Trophies', value: String(p.trophies ?? 0), inline: true }, { name: 'Best', value: String(p.bestTrophies ?? 0), inline: true }, { name: 'War Stars', value: String(p.warStars ?? 0), inline: true }, { name: 'Heroes', value: heroSummary(p), inline: false }, { name: 'Clan', value: p.clan ? `${p.clan.name} (${p.clan.tag})` : 'No clan', inline: false }); return reply({ embeds: [e] }); } const th = Number(p.townHallLevel || 0), heroes = (p.heroes || []).filter((h: any) => h.village === 'home'), heroLevels = heroes.reduce((n: number, h: any) => n + Number(h.level || 0), 0), score = Math.min(100, Math.round(th * 4 + heroLevels * .45 + Math.min(20, Number(p.warStars || 0) / 50))); const verdict = score >= 75 ? '🟢 Excellent' : score >= 55 ? '🟡 Review' : '🔴 Weak'; const e = new EmbedBuilder().setTitle(`🔎 Recruitment Check • ${p.name}`).setDescription(`${verdict} • Score **${score}/100**`).addFields({ name: 'Town Hall', value: `TH${th}`, inline: true }, { name: 'Trophies', value: String(p.trophies ?? 0), inline: true }, { name: 'Hero levels', value: String(heroLevels), inline: true }, { name: 'War stars', value: String(p.warStars ?? 0), inline: true }, { name: 'Recommendation', value: score >= 75 ? 'Strong candidate' : score >= 55 ? 'Review war activity and upgrades' : 'Below typical competitive threshold', inline: false }); return reply({ embeds: [e] }); }
  if (sub === 'war') return reply({ embeds: [fmtWar(await getCurrentWar(tag))] });
  if (sub === 'warlog') { const w = await getWarLog(tag); const rows = (w.items || []).slice(0, 5).map((x: any) => `${x.result === 'win' ? '🟢' : x.result === 'lose' ? '🔴' : '⚪'} ${x.clan?.name || 'Clan'} vs ${x.opponent?.name || 'Opponent'} • ${x.clan?.stars ?? 0}-${x.opponent?.stars ?? 0}`).join('\n') || 'No war history available.'; return reply({ embeds: [new EmbedBuilder().setTitle('📜 Recent War Log').setDescription(rows)] }); }
  if (sub === 'members') { const c = await getClan(tag); const rows = (c.memberList || []).slice(0, 25).map((m: any) => `**TH${m.townHallLevel}** ${m.name} • ${m.trophies ?? 0} 🏆`).join('\n') || 'No members returned.'; return reply({ embeds: [new EmbedBuilder().setTitle(`👥 ${c.name} Roster`).setDescription(rows).setFooter({ text: `${c.members}/50 members • showing up to 25` })] }); }
  const group = await getCwlLeague(tag); const clans = (group.clans || []).map((c: any) => `${c.tag} • ${c.name} • Level ${c.clanLevel ?? '?'}`).join('\n') || 'No CWL group available.'; return reply({ embeds: [new EmbedBuilder().setTitle(`🏆 CWL Group • ${group.season || 'Current'}`).setDescription(clans).setFooter({ text: 'Use /coc war for the live war when available.' })] });
}
...
async function executeSlash(i: any) {
  ...
  await i.deferReply();
  try { return i.editReply(await renderAction(sub, tag, i.options.getString('tag') || '', (payload) => payload)); }
  catch (e) { return i.editReply({ content: `❌ ${fail(e)}` }); }
}

async function executePrefix(m: any, args: string[]) {
  if (!cocConfigured()) return m.reply('⚠️ CoC API is not configured. Set `COC_API_TOKEN` in the bot environment.');
  const sub = String(args[0] || '').toLowerCase();
  const value = args[1] || '';
  const valid = ['clan', 'player', 'war', 'warlog', 'cwl', 'members', 'recruit'];
  if (!valid.includes(sub)) return m.reply('Usage: !coc <clan|player|war|warlog|cwl|members|recruit> [tag]');
  const tag = ['player', 'recruit'].includes(sub) ? value : value || process.env.COC_DEFAULT_CLAN_TAG || '';
  if (['player', 'recruit'].includes(sub) && !tag) return m.reply(`Usage: !coc ${sub} <player-tag>`);
  try { return m.reply(await renderAction(sub, tag, tag, (payload) => payload)); }
  catch (e) { return m.reply(`❌ ${fail(e)}`); }
}

export = { name: 'coc', description: 'Clash of Clans tools for clan leaders, members, wars, CWL, and recruitment.', data, executeSlash, executePrefix };
