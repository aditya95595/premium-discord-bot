...
function prune(key:string,now:number){
  const a=(recent.get(key)||[]).filter(x=>now-x.ts<10000).slice(-12);
  recent.set(key,a);
  return a;
}
...
export async function runAutoMod(message:Message){
  ...
  const punish=async(type:string,reason:string)=>{
    ...
    logWarn('AutoMod action',{guild:message.guild!.id,user:message.author.id,type});
  };
  const normalized=c.trim().toLowerCase().replace(/\s+/g,' ');
  ...
  arr.push({ts:now,text:normalized});
  recent.set(key,arr);
  ...
  if(blocked.some(w=>lower.includes(w)))return punish('automod-word','blocked word');
  if(s.automod_caps){
    const letters=c.replace(/[^A-Za-z]/g,'');
    const caps=letters.replace(/[^A-Z]/g,'').length;
    if(letters.length>=10&&caps/letters.length>.85)return punish('automod-caps','excessive caps');
  }
}

setInterval(()=>{
  const cutoff=Date.now()-30000;
  for(const[k,v]of recent)if(!v.length||v[v.length-1].ts<cutoff)recent.delete(k);
},15000).unref();
