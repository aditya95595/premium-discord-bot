...
const auditType:Record<string,AuditLogEvent>={CHANNEL_DELETE:AuditLogEvent.ChannelDelete,ROLE_DELETE:AuditLogEvent.RoleDelete,BAN:AuditLogEvent.MemberBanAdd,KICK:AuditLogEvent.MemberKick,CHANNEL_UPDATE:AuditLogEvent.ChannelUpdate,ROLE_UPDATE:AuditLogEvent.RoleUpdate,PERMISSION_UPDATE:AuditLogEvent.ChannelOverwriteCreate};
...
async function lockdown(guild:Guild,reason:string){
  const me=guild.members.me;if(!me?.permissions.has(PermissionFlagsBits.ManageChannels))return false;
  ...
  info(`Security lockdown enabled for ${guild.id}: ${reason}`);
  return true;
}
...
export async function recordDestructiveAction(guild:Guild,action:string,targetId?:string){
  try{
    ...
    if(mode==='lockdown'||mode==='ban_and_lockdown')await lockdown(guild,`threshold reached by ${executorId}`);
