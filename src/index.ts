...
client.on(Events.ChannelUpdate,(oldChannel,newChannel)=>{if(newChannel.guild)void recordDestructiveAction(newChannel.guild,'CHANNEL_UPDATE',newChannel.id);});
client.on(Events.GuildRoleDelete,role=>void recordDestructiveAction(role.guild,'ROLE_DELETE',role.id));
client.on(Events.GuildRoleUpdate,(oldRole,newRole)=>void recordDestructiveAction(newRole.guild,'ROLE_UPDATE',newRole.id));
client.on(Events.GuildBanAdd,ban=>void recordDestructiveAction(ban.guild,'BAN',ban.user.id));
client.on(Events.GuildMemberRemove,member=>{if(member.guild)void recordDestructiveAction(member.guild,'KICK',member.id);});
process.on('unhandledRejection',e=>error('UnhandledRejection',e));
process.on('uncaughtException',e=>{error('UncaughtException',e);process.exitCode=1;});
