...
function parsePrefix(content: string, prefix: string) {
  if (!content.startsWith(prefix)) return null;
  const body = content.slice(prefix.length).trim();
  if (!body) return null;
  const args = body.match(/(?:[^\s"]+|"(?:\\.|[^"\\])*")+/g) ?? [];
  const name = (args.shift() ?? '').toLowerCase();
  return { name, args: args.map(x => x.replace(/^"|"$/g, '')) };
}

function cooldownLeft(source: any, command: Command) {
  const seconds = command.cooldown ?? 2;
  if (!source.guild || seconds <= 0) return 0;
  const userId = source.user?.id ?? source.author?.id;
  const key = `${source.guild.id}:${userId}:${command.name}`;
  const now = Date.now();
...
setInterval(() => { const cutoff = Date.now() - 15 * 60_000; for (const [k, v] of cooldowns) if (v < cutoff) cooldowns.delete(k); }, 5 * 60_000).unref();
...
async function executeCommand(source: any, command: Command, args: string[] = []) {
  ...
  try {
    if (source.isChatInputCommand?.()) return command.executeSlash ? await command.executeSlash(source) : source.reply({ content: 'This command is not available as a slash command.', ephemeral: true });
    return command.executePrefix ? await command.executePrefix(source, args) : source.reply('This command is not available with the prefix.');
  } catch (e) {
    error(`Command ${command.name} failed`, e instanceof Error ? e.message : String(e));
    return source.reply({ content: 'An unexpected error occurred while executing the command.', ephemeral }).catch(() => {});
  }
}

client.once(Events.ClientReady, async ready => {
  info(`Logged in as ${ready.user.tag}`);
  info(`Guild count: ${ready.guilds.cache.size}`);
  await applyStatusSettings(ready).catch(e => error('Status setup error', e));
});
...
client.on(Events.MessageCreate, async message => {
  try {
    ...
    const parsed = parsePrefix(message.content, getGuildSettings(message.guild.id).prefix || '!');
    if (!parsed) return;
    const command = commands.get(parsed.name);
    if (command) await executeCommand(message, command, parsed.args);
  } catch (e) { error('Message handler error', e instanceof Error ? e.message : String(e)); }
});
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    ...
    error('Interaction handler error', e instanceof Error ? e.message : String(e));
    if (!interaction.replied && !interaction.deferred) await interaction.reply({ content: 'An unexpected error occurred.', ephemeral: true }).catch(() => {});
  }
});
...
async function start() {
  ...
  const shutdown = () => { clearStatusTimers(); server.close(); client.destroy(); db.close(); };
