...
client.once(Events.ClientReady,async ready=>{info(`Logged in as ${ready.user.tag}`);info(`Guild count: ${ready.guilds.cache.size}`);startReminderService(ready);await applyStatusSettings(ready).catch(e=>error('Status setup error',e));});
