...
client.on(Events.GuildMemberRemove,async member=>{if(!member.guild)return;const executor=await confirmKick(member.guild,member.id);if(executor)void recordDestructiveAction(member.guild,'KICK',member.id);});
